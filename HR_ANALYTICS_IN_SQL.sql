...
-- 6. EMPLOYEE ATTRIION AS PER AGE_GROUP
	    ...
	    select `AgeGroup`,COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
        FROM`analytics_data`
        group by AgeGroup
...
select Department,COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
group by Department
ORDER BY COUNT(`EmpID`) DESC;
...
select DistanceFromHome_SLABWISE,COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
group by DistanceFromHome_SLABWISE
ORDER BY COUNT(`EmpID`) DESC;
...
select EducationField, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
group by EducationField
ORDER BY COUNT(`EmpID`) DESC;
...
select Gender, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
group by Gender
ORDER BY COUNT(`EmpID`) DESC;
...
select EnvironmentSatisfaction, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
...
select JobInvolvement, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
...
select PerformanceRating, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
...
select YearsAtCompany_SLABWISE, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
group by YearsAtCompany_SLABWISE
ORDER BY COUNT(`EmpID`) DESC;


-- 15. EMPLOYEE ATTRITION AS PER YearsSinceLastPromotion_LST PROMOTION


select WorkLifeBalance, COUNT(`EmpID`),COUNT(`ATTRITION_STATUS`)
FROM`analytics_data`
